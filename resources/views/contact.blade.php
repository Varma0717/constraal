...
@section('title', 'Contact Constraal')
            ...
            <div class="section-card p-8">
                ...
                <form method="POST" action="{{ route('contact.submit') }}" class="mt-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" required>
                        @error('name') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="email" class="block text-sm font-medium mb-2">Email</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" required>
                        @error('email') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="company" class="block text-sm font-medium mb-2">Company (Optional)</label>
                        <input type="text" id="company" name="company" value="{{ old('company') }}" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent">
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="phone" class="block text-sm font-medium mb-2">Phone (Optional)</label>
                            <input type="text" id="phone" name="phone" value="{{ old('phone') }}" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" placeholder="+1 555 123 4567">
                            @error('phone') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="country" class="block text-sm font-medium mb-2">Country/Region (Optional)</label>
                            <input type="text" id="country" name="country" value="{{ old('country') }}" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" placeholder="United States">
                    ...
                    <div>
                        ...
                        <select id="inquiry_type" name="inquiry_type" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" required>
                            ...
                            <option value="press" {{ old('inquiry_type') === 'press' ? 'selected' : '' }}>Press & Media</option>
                            <option value="early_interest" {{ old('inquiry_type') === 'early_interest' ? 'selected' : '' }}>Early Interest</option>
                        </select>
                        @error('inquiry_type') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('preferred_contact') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="use_case" class="block text-sm font-medium mb-2">Project Context (Optional)</label>
                        <textarea id="use_case" name="use_case" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" placeholder="Tell us what you’re trying to build or solve.">{{ old('use_case') }}</textarea>
                        @error('use_case') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="message" class="block text-sm font-medium mb-2">Message</label>
                        <textarea id="message" name="message" rows="5" class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-accent" required>{{ old('message') }}</textarea>
                ...
                <div class="section-card p-8">
                    ...
                    <div class="mt-6 space-y-6">
                        ...
                        <div>
                            ...
                            <a href="{{ route('careers') }}" class="text-accent hover:text-accent transition-colors">View open positions →</a>
                        </div>
                        <div>
